...
exports.run = async (client, message, args) => {
  ...
  if (args[0] === 'reset') {
    ...
    //if (!isNaN) return message.channel.send('<:gierro:710197544751202414> » Você deve mencionar um canal!')
    if (isNaN(args[0])){
        return message.reply(`<:gierro:710197544751202414> » Você deve me indicar o id de um canal`)
}
  
    console.log(`canal de welcome member setado com sucesso: ${args[0]} no servidor: ${message.guild.name} id: ${message.guild.id}`)
  ...
  message.channel.send(embedi).then(msg => {
    ...
    msg.awaitReactions(filter, { max: 1, time: 60000, errors: ['time']}) 
      ...
      if (reaction.emoji.id === '710198069068562473') {
          msg.delete()
          message.channel.send('<:gicerto:710198069068562473> » Você definiu o canal de bem vindo em: '+`<#${args[0]}>`); 
          db.set(`canalb_${message.guild.id}`, args[0])
        }
    ...
    //message.channel.send('<:gicerto:710198069068562473> » canal de welcome setado com sucesso:\n'+`<#${anal}>`) 
