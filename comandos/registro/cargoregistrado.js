...
exports.run = async (client, message, args) => {
  ...
  if (prefixos === null) prefixos = `${config.prefixos}`
  ...
  .setDescription(`Você tem certeza que deseja colocar o cargo: ${lrole}, como o cargo de registrado?`)
  
  message.channel.send({embed}).then(msg => { // definindo a função 'then' como 'msg'

     msg.react('710198069068562473').then(() => msg.react('700492899833479249'))

    const filter = (reaction, user) => { // Criando um filtro para quem clicou no emoji
      return ['710198069068562473', '700492899833479249'].includes(reaction.emoji.id) && user.id === message.author.id;
    };
    ...
    msg.awaitReactions(filter, { max: 1, time: 60000, errors: ['time']})
      ...
      //cor rosa
        if (reaction.emoji.id === '710198069068562473') {//<:gicerto:710198069068562473>
          ...
          message.reply('<:gicerto:710198069068562473> » Você definiu o cargo '+ `${lrole}`+' como o cargo de registrado'); 
